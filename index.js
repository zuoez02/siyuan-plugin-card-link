...
module.exports = class CardLinkApi extends Plugin {
  onload() {
    this.protyleSlash = [
      {
        filter: ["Card Link", "插入卡片链接", "kplj"],
        html: `<div class="b3-list-item__first"><span class="b3-list-item__text">${this.i18n.insertTitle}</span><span class="b3-list-item__meta"></span></div>`,
        id: "insertCardLink",
  ...
  async generateCardHTML(config) {
    ...
    try {
      const url = await this.openDialog(conf);
      if (url) {
        const newConf = await this.microlinkScraper(url);
  ...
  openDialog(config) {
    return new Promise((resolve, reject) => {
      const dialog = new Dialog({
        title: this.i18n.insertTitle,
        content: `<div class="b3-dialog__content"><textarea class="b3-text-field fn__block" placeholder="URL"></textarea></div>
